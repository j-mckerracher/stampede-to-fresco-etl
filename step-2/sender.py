...
SERVER_COMPLETE_DIR = Path(r"U:\projects\stampede-step-3\complete") # Needed to check for processing jobs
SERVER_OUTPUT_DIR = Path(r"U:\projects\stampede-step-3\output") # Not strictly needed by sender, but kept for context
DESTINATION_DIR = Path(r"C:\Users\jmckerra\Documents\Stampede\step-3-complete") # Needed to check already completed
...
def check_pending_manifests(logger):
            ...
            if file.is_file() and file.name.endswith(".manifest.json"):
                try:
                    ...
                    if job_id:
                        ...
                        logger.debug(f"Found pending manifest for folder {year_month} (Job ID: {job_id})")
    ...
    logger.debug(f"Pending check found {len(pending_folders)} unique folders and {len(pending_job_ids)} unique job IDs.")
    return pending_folders, pending_job_ids
...
def check_processing_jobs_by_status(logger):
            ...
            if file.is_file() and file.name.endswith(".status"):
                try:
                    ...
                    if status == "processing" and year_month and job_id:
                        ...
                        logger.debug(f"Consumer is actively processing folder {year_month} (job {job_id})")
                except json.JSONDecodeError:
    ...
    logger.debug(f"Processing check found {len(processing_folder_names)} unique folders and {len(processing_job_ids)} unique job IDs.")
    return processing_folder_names, processing_job_ids
    ...
    try:
        ...
        for item in DESTINATION_DIR.iterdir():
            if item.is_dir():
                ...
                if any(item.glob("*.parquet")): # Check for presence of parquet files
                    completed_folders.add(item.name)
                    logger.debug(f"Found successfully completed folder in destination: {item.name}")
                else:
                     logger.debug(f"Found folder in destination, but no parquet files: {item.name}. Treating as incomplete.")


    except Exception as e:
        logger.error(f"Error checking final destination directory {DESTINATION_DIR}: {str(e)}")

    logger.debug(f"Destination check found {len(completed_folders)} potentially completed folders.")
    return completed_folders
...
def copy_files_to_input(source_folder, logger):
    ...
    try:
        ...
        # Ensure we are looking for the correct pattern
        ...
        if accounting_file_path.exists() and accounting_file_path.is_file():
            ...
            accounting_file_details = {"name": accounting_file_name, "path": str(dest_accounting_file)} # Use string for path in dict
        else:
            logger.warning(f"Accounting file {accounting_file_path} not found or is not a file for {folder_name}. Proceeding without it.")
...
def distribute_jobs(logger):
    ...
    logger.info(f"Current state: {len(pending_folders)} pending folders (manifests), {len(processing_folders)} processing folders (status), {len(completed_folders_dest)} folders in final destination.")
    logger.info(f"Calculated active/pending job count: {current_active_job_count}. Max allowed: {MAX_ACTIVE_JOBS}.")
    logger.debug(f"Folders to skip (already active or completed): {folders_to_skip}")

    # 2. Check if capacity allows for new jobs
    if current_active_job_count >= MAX_ACTIVE_JOBS:
        logger.info(f"Job capacity ({MAX_ACTIVE_JOBS}) reached or exceeded. Waiting for jobs to complete before submitting more.")
    ...
    for folder in sorted(source_folders, key=lambda f: f.name):
        ...
        if not any(folder.glob("sorted_*.parquet")):
            logger.debug(f"Skipping folder {year_month}: No 'sorted_*.parquet' files found inside.")
            # Log if unsorted files exist, indicating a prerequisite step might be needed
            if any(folder.glob("perf_metrics_*.parquet")):
                 logger.info(f"Folder {year_month} contains unsorted 'perf_metrics_*.parquet' files. Ensure sorting process is complete.")
            continue # Skip this folder

        logger.info(f"Processing eligible source folder: {year_month}")
        ...
        if job_id:
            ...
            logger.error(f"Failed to create manifest for {year_month} AFTER copying files. Manual cleanup of files in {SERVER_INPUT_DIR} might be needed for {year_month}.")
