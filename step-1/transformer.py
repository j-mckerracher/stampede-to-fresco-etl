...
REQUEST_TIMEOUT = 60
WRITE_BATCH_SIZE = 100  # Number of NODES to process before writing
# --- End Configuration ---
...
def process_single_node(node_name: str, node_url: str, processor: NodeDataProcessor, temp_base_dir: Path) -> Optional[pl.DataFrame]:
    ...
    try:
        ...
        if node_df is None or node_df.height == 0:
            ...
            if 'Timestamp' not in node_df.columns:
                logger.error(f"CRITICAL: 'Timestamp' column missing in data from {node_name} processor. Cannot proceed with this node.")
            ...
            # Optional: Add more robust type checking/conversion if needed
                 ...
                 logger.warning(f"Timestamp column for {node_name} is not datetime type ({type(node_df['Timestamp'][0])}). Attempting Polars conversion.")
                 try:
                     ...
                     logger.error(f"Failed to convert Timestamp column for {node_name}: {e}. Skipping node data.", exc_info=True)
                     processed_df = None
                 else:
...
def write_daily_batch_data(
    ...
    for date_str, list_of_daily_dfs in daily_data_aggregator.items():
        ...
        if not list_of_daily_dfs:
            logger.warning(f"No DataFrames found for date {date_str} in batch {batch_number}, skipping write for this day.")
            continue
        ...
        try:
            ...
            logger.debug(f"Concatenating {len(list_of_daily_dfs)} DataFrame(s) for day {date_str}, batch {batch_number}...")
            ...
            logger.debug(f"Saving data for batch {batch_number}, date {date_str} ({combined_daily_df.height} rows) to {output_file}")
    ...
    logger.info(f"--- Finished writing batch {batch_number} in {batch_end_time - batch_start_time:.2f}s [{status_str}] ---")
...
def main():
    ...
    for i, (node_name, node_url) in enumerate(nodes_to_process):
        ...
        try:
            ...
            if node_data is not None and node_data.height > 0:
                ...
                try:
                    ...
                    daily_partitions = node_data_with_date.partition_by('date_str', as_dict=True, include_key=False) # include_key=False drops 'date_str'
                    ...
                    for date_key, daily_df in daily_partitions.items():
                         if daily_df is not None and daily_df.height > 0:
                              ...
                              logger.debug(f"Adding {daily_df.height} rows for date {date_key_str} from node {node_name} to aggregator.")
        ...
        if nodes_processed_in_batch >= WRITE_BATCH_SIZE:
            logger.info(f"Node batch size ({WRITE_BATCH_SIZE}) reached. Writing aggregated daily data for batch {current_batch_number}...")
    ...
    if daily_data_aggregator:
        logger.info(f"Writing final remaining batch {current_batch_number} ({nodes_processed_in_batch} nodes processed in this partial batch)...")
        if not write_daily_batch_data(daily_data_aggregator, current_batch_number, FINAL_OUTPUT_DIR):
