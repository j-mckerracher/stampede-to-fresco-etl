...
PROCESS_ERROR_FILES = True  # Set to True to process error files at startup
...
class ParquetHandler(FileSystemEventHandler):
    def __init__(self, watch_folder, processing_folder, output_folder, error_folder, executor, max_retries=MAX_RETRIES,
        ...
        self._error_recovery_folder = os.path.join(self.processing_folder,
                                                   "error_recovery")  # Folder for processing error files

        # Create folders if they don't exist
        for folder in [watch_folder, processing_folder, output_folder, error_folder, self._error_recovery_folder]:
            try:
        ...
        log_info(
            f"Initialized ParquetHandler:\n  Watch: {watch_folder}\n  Processing: {processing_folder}\n  Output: {output_folder}\n  Error: {error_folder}\n  Error Recovery: {self._error_recovery_folder}\n  Workers: {MAX_WORKERS}")

    def process_error_file(self, error_file_path):
        """Process a file from the error folder and attempt to append it to the target"""
        thread_id = threading.get_ident()
        error_filename = os.path.basename(error_file_path)
        log_info(f"[Thread-{thread_id}] Processing error file: {error_filename}")

        # Extract the original date part from the error filename
        # Example: 2013-02-12.parquet.6c8669f8.error -> 2013-02-12.parquet
        match = re.search(r"([\d-]+\.parquet)", error_filename)
        if not match:
            log_warning(f"[Thread-{thread_id}] Could not extract date from error filename: {error_filename}. Skipping.")
            return

        standardized_filename = match.group(1)
        unique_id = uuid.uuid4().hex[:8]
        recovery_name = f"{os.path.splitext(standardized_filename)[0]}_{unique_id}.recovery.tmp"
        recovery_path = os.path.join(self._error_recovery_folder, recovery_name)

        try:
            # Move to recovery folder
            log_info(f"[Thread-{thread_id}] Moving error file to recovery: {error_file_path} -> {recovery_path}")
            shutil.copy2(error_file_path, recovery_path)

            # Process the file using the existing append method
            target_lock = self.get_target_lock(standardized_filename)
            with target_lock:
                log_info(f"[Thread-{thread_id}] Acquired lock for {standardized_filename} (error recovery)")
                self.append_to_target_pandas(recovery_path, standardized_filename, unique_id)

            # If successful, remove the original error file and the recovery file
            os.remove(error_file_path)
            log_info(f"[Thread-{thread_id}] Successfully processed and removed error file: {error_filename}")
            os.remove(recovery_path)
            log_info(f"[Thread-{thread_id}] Removed recovery file: {recovery_path}")

        except Exception as e:
            log_error(f"[Thread-{thread_id}] Failed to process error file {error_filename}: {e}")
            # Clean up recovery file if it exists
            if os.path.exists(recovery_path):
                try:
                    os.remove(recovery_path)
                    log_info(f"[Thread-{thread_id}] Removed failed recovery file: {recovery_path}")
                except OSError as rm_err:
                    log_warning(f"[Thread-{thread_id}] Could not remove failed recovery file {recovery_path}: {rm_err}")
...
def process_error_files(error_folder, handler):
    """Scans the error folder for errored Parquet files and processes them."""
    log_info(f"Scanning for error files in {error_folder}...")
    count = 0
    try:
        for filename in os.listdir(error_folder):
            if filename.lower().endswith('.error'):
                ...
                if os.path.isfile(error_file_path):
                    count += 1
                    log_info(f"Found error file: {error_file_path}")
                    # Submit to the thread pool for processing
                    handler.executor.submit(handler.process_error_file, error_file_path)
        log_info(f"Submitted {count} error files for reprocessing.")
    except FileNotFoundError:
        log_error(f"Error folder {error_folder} not found during initial scan.")
    except OSError as e:
        log_error(f"Error listing files in error folder {error_folder}: {e}")
...
def start_monitoring(watch_folder, processing_folder, output_folder, error_folder, process_error_files=True):
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS, thread_name_prefix='ParquetWorker') as executor:
        ...
        # Process error files if requested
        if process_error_files:
            log_info("Processing files from the error folder...")
            process_error_files(error_folder, event_handler)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Parquet file processing from error folder')
    parser.add_argument('--monitor', action='store_true',
                        help='Start monitoring mode instead of just processing error files')
    args = parser.parse_args()

    # By default, only process error files
    if args.monitor:
        # If monitor flag is provided, start normal monitoring with error processing
        log_info("Starting in monitoring mode with error processing...")
        start_monitoring(WATCH_FOLDER, PROCESSING_FOLDER, OUTPUT_FOLDER, ERROR_FOLDER, process_error_files=True)
    else:
        # Default mode: only process error files then exit
        log_info("Starting in error-processing-only mode...")
        with ThreadPoolExecutor(max_workers=MAX_WORKERS, thread_name_prefix='ParquetErrorWorker') as executor:
            handler = ParquetHandler(WATCH_FOLDER, PROCESSING_FOLDER, OUTPUT_FOLDER, ERROR_FOLDER, executor)
            process_error_files(ERROR_FOLDER, handler)
            log_info("Error files processing complete. Exiting.")